...
func (gq *GroupQuery) QueryDevices() *DeviceQuery {
...
func (gq *GroupQuery) QueryDomain() *DomainQuery {
	query := (&DomainClient{config: gq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := gq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (gq *GroupQuery) FirstID(ctx context.Context) (id xid.ID, err error) {
	var ids []xid.ID
...
func (gq *GroupQuery) FirstIDX(ctx context.Context) xid.ID {
...
func (gq *GroupQuery) OnlyID(ctx context.Context) (id xid.ID, err error) {
	var ids []xid.ID
...
func (gq *GroupQuery) OnlyIDX(ctx context.Context) xid.ID {
...
func (gq *GroupQuery) IDs(ctx context.Context) (ids []xid.ID, err error) {
...
func (gq *GroupQuery) IDsX(ctx context.Context) []xid.ID {
	ids, err := gq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (gq *GroupQuery) Clone() *GroupQuery {
	...
	return &GroupQuery{
		...
		withDomain:  gq.withDomain.Clone(),
...
func (gq *GroupQuery) WithDevices(opts ...func(*DeviceQuery)) *GroupQuery {
	query := (&DeviceClient{config: gq.config}).Query()
...
func (gq *GroupQuery) WithDomain(opts ...func(*DomainQuery)) *GroupQuery {
	query := (&DomainClient{config: gq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (gq *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := gq.withDevices; query != nil {
		if err := gq.loadDevices(ctx, query, nodes,
			func(n *Group) { n.Edges.Devices = []*Device{} },
			func(n *Group, e *Device) { n.Edges.Devices = append(n.Edges.Devices, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := gq.withDomain; query != nil {
		if err := gq.loadDomain(ctx, query, nodes, nil,
			func(n *Group, e *Domain) { n.Edges.Domain = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (gq *GroupQuery) loadDevices(ctx context.Context, query *DeviceQuery, nodes []*Group, init func(*Group), assign func(*Group, *Device)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[xid.ID]*Group)
	nids := make(map[xid.ID]map[*Group]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(group.DevicesTable)
		s.Join(joinT).On(s.C(device.FieldID), joinT.C(group.DevicesPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(group.DevicesPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(group.DevicesPrimaryKey[0]))
...
func (gq *GroupQuery) loadDomain(ctx context.Context, query *DomainQuery, nodes []*Group, init func(*Group), assign func(*Group, *Domain)) error {
	ids := make([]xid.ID, 0, len(nodes))
	nodeids := make(map[xid.ID][]*Group)
	for i := range nodes {
		if nodes[i].domain_groups == nil {
			continue
		}
		fk := *nodes[i].domain_groups
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "domain_groups" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
