...
type DomainQuery struct {
	...
	withChilddomains *DomainQuery
	withParentdomain *DomainQuery
	withFKs          bool
...
func (dq *DomainQuery) QueryDevices() *DeviceQuery {
...
func (dq *DomainQuery) QueryGroups() *GroupQuery {
...
// QueryChilddomains chains the current query on the "childdomains" edge.
func (dq *DomainQuery) QueryChilddomains() *DomainQuery {
...
// QueryParentdomain chains the current query on the "parentdomain" edge.
func (dq *DomainQuery) QueryParentdomain() *DomainQuery {
	query := (&DomainClient{config: dq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := dq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (dq *DomainQuery) WithDevices(opts ...func(*DeviceQuery)) *DomainQuery {
	query := (&DeviceClient{config: dq.config}).Query()
...
func (dq *DomainQuery) WithGroups(opts ...func(*GroupQuery)) *DomainQuery {
	query := (&GroupClient{config: dq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithChilddomains tells the query-builder to eager-load the nodes that are connected to
// the "childdomains" edge. The optional arguments are used to configure the query builder of the edge.
func (dq *DomainQuery) WithChilddomains(opts ...func(*DomainQuery)) *DomainQuery {
	query := (&DomainClient{config: dq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	dq.withChilddomains = query
	return dq
}

// WithParentdomain tells the query-builder to eager-load the nodes that are connected to
// the "parentdomain" edge. The optional arguments are used to configure the query builder of the edge.
func (dq *DomainQuery) WithParentdomain(opts ...func(*DomainQuery)) *DomainQuery {
	query := (&DomainClient{config: dq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	dq.withParentdomain = query
	return dq
}
...
func (dq *DomainQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Domain, error) {
	var (
		...
		loadedTypes = [5]bool{
			...
			dq.withUsers != nil,
			dq.withGroups != nil,
			dq.withChilddomains != nil,
			dq.withParentdomain != nil,
		}
	)
	if dq.withParentdomain != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := dq.withDevices; query != nil {
		if err := dq.loadDevices(ctx, query, nodes,
			func(n *Domain) { n.Edges.Devices = []*Device{} },
			func(n *Domain, e *Device) { n.Edges.Devices = append(n.Edges.Devices, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := dq.withGroups; query != nil {
		if err := dq.loadGroups(ctx, query, nodes,
			func(n *Domain) { n.Edges.Groups = []*Group{} },
			func(n *Domain, e *Group) { n.Edges.Groups = append(n.Edges.Groups, e) }); err != nil {
			return nil, err
		}
	}
	if query := dq.withChilddomains; query != nil {
		if err := dq.loadChilddomains(ctx, query, nodes,
			func(n *Domain) { n.Edges.Childdomains = []*Domain{} },
			func(n *Domain, e *Domain) { n.Edges.Childdomains = append(n.Edges.Childdomains, e) }); err != nil {
			return nil, err
		}
	}
	if query := dq.withParentdomain; query != nil {
		if err := dq.loadParentdomain(ctx, query, nodes, nil,
			func(n *Domain, e *Domain) { n.Edges.Parentdomain = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (dq *DomainQuery) loadDevices(ctx context.Context, query *DeviceQuery, nodes []*Domain, init func(*Domain), assign func(*Domain, *Device)) error {
	...
	query.Where(predicate.Device(func(s *sql.Selector) {
		s.Where(sql.InValues(domain.DevicesColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.domain_devices
		if fk == nil {
			return fmt.Errorf(`foreign-key "domain_devices" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "domain_devices" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (dq *DomainQuery) loadUsers(ctx context.Context, query *UserQuery, nodes []*Domain, init func(*Domain), assign func(*Domain, *User)) error {
	...
	for _, n := range neighbors {
		fk := n.domain_users
		if fk == nil {
			return fmt.Errorf(`foreign-key "domain_users" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "domain_users" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (dq *DomainQuery) loadGroups(ctx context.Context, query *GroupQuery, nodes []*Domain, init func(*Domain), assign func(*Domain, *Group)) error {
	...
	query.Where(predicate.Group(func(s *sql.Selector) {
		s.Where(sql.InValues(domain.GroupsColumn, fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.domain_groups
		if fk == nil {
			return fmt.Errorf(`foreign-key "domain_groups" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "domain_groups" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (dq *DomainQuery) loadChilddomains(ctx context.Context, query *DomainQuery, nodes []*Domain, init func(*Domain), assign func(*Domain, *Domain)) error {
	...
	query.Where(predicate.Domain(func(s *sql.Selector) {
		s.Where(sql.InValues(domain.ChilddomainsColumn, fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.domain_childdomains
		if fk == nil {
			return fmt.Errorf(`foreign-key "domain_childdomains" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "domain_childdomains" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (dq *DomainQuery) loadParentdomain(ctx context.Context, query *DomainQuery, nodes []*Domain, init func(*Domain), assign func(*Domain, *Domain)) error {
	...
	for i := range nodes {
		...
		fk := *nodes[i].domain_childdomains
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "domain_childdomains" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
