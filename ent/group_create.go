...
func (gc *GroupCreate) SetNillablePermissions(s *string) *GroupCreate {
	if s != nil {
		gc.SetPermissions(*s)
	}
	return gc
...
func (gc *GroupCreate) SetDomain(d *Domain) *GroupCreate {
	return gc.SetDomainID(d.ID)
}
...
func (gc *GroupCreate) createSpec() (*Group, *sqlgraph.CreateSpec) {
	...
	if nodes := gc.mutation.DomainIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2O,
			Inverse: true,
