...
func (uq *UserQuery) QueryRodents() *RodentQuery {
	query := (&RodentClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withRodents: uq.withRodents.Clone(),
		withGroups:  uq.withGroups.Clone(),
		withDomain:  uq.withDomain.Clone(),
...
func (uq *UserQuery) WithRodents(opts ...func(*RodentQuery)) *UserQuery {
	query := (&RodentClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withRodents = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withRodents; query != nil {
		if err := uq.loadRodents(ctx, query, nodes,
			func(n *User) { n.Edges.Rodents = []*Rodent{} },
			func(n *User, e *Rodent) { n.Edges.Rodents = append(n.Edges.Rodents, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := uq.withDomain; query != nil {
		if err := uq.loadDomain(ctx, query, nodes, nil,
			func(n *User, e *Domain) { n.Edges.Domain = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadDevices(ctx context.Context, query *DeviceQuery, nodes []*User, init func(*User), assign func(*User, *Device)) error {
	...
	query.Where(func(s *sql.Selector) {
		...
		s.Join(joinT).On(s.C(device.FieldID), joinT.C(user.DevicesPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(user.DevicesPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.DevicesPrimaryKey[1]))
...
func (uq *UserQuery) loadRodents(ctx context.Context, query *RodentQuery, nodes []*User, init func(*User), assign func(*User, *Rodent)) error {
	...
	query.Where(predicate.Rodent(func(s *sql.Selector) {
		s.Where(sql.InValues(user.RodentsColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.user_rodents
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_rodents" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_rodents" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (uq *UserQuery) loadDomain(ctx context.Context, query *DomainQuery, nodes []*User, init func(*User), assign func(*User, *Domain)) error {
	ids := make([]xid.ID, 0, len(nodes))
	nodeids := make(map[xid.ID][]*User)
	for i := range nodes {
		if nodes[i].domain_users == nil {
			continue
		}
		fk := *nodes[i].domain_users
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "domain_users" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
